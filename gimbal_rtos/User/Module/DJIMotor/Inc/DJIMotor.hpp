...
public:
    ...
    /* 位置值转换为弧度的转换因子，编码器为十三位，2^13-1 = 8191, 2*PI / 8191 (rad)，
    这样，当编码器的值增加或减少 1 时，它表示电机轴旋转了 2*PI / 8191 (rad) */
    ...
    struct GearRatio_PI
    {
        static constexpr float None = Numeric::Pi;       /*!< PI */
    ...
    PID speedPid = PID(0.1f, 0.0f, 0.0f, 25000.0f, 100.0f, PID_POSITION);    ///< 速度环PID控制器
    PID positionPid = PID(0.1f, 0.0f, 0.0f, 500.0f, 10.0f, PID_POSITION); ///< 位置环PID控制器
